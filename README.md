...
you either way, by email and iMessage.
...
MindBody login, `SMTP_USER` / `SMTP_PASS` / `NOTIFY_EMAIL` for email alerts, and
`IMSG_URL` to also get texted. Two accounts are supported (`MB2_*`), switchable
