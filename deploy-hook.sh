#!/bin/sh
# Deploy the moment a push lands on main, instead of waiting for the poller.
#
...
# receiver process and no shared secret. Every event for main runs
...
gh webhook forward --events=push --repo="$REPO" | while IFS= read -r line; do
  case $line in
    *refs/heads/main*)
      echo "=== $(date) push to main"
      sh ./autodeploy.sh || true    # keep listening even if one deploy fails
      ;;
  esac
done
